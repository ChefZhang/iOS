...
那么如果我如果放在viewDidLoad中呢？ 这个确实是一个解决办法！
还有没有更好的办法呢？因为你要知道，如果你这么做，意味着你以后每个控件都是加载到viewDidLoad中，这样就导致app大家，加载画面特别卡，是不是这样？！
要是只有当我用到它时，它才被加载进来，是不是很好，不用，我就永远不加载，那用到它的时候，是不是就意味着，调去getter方法，那只要我们重写getter方法就行了，是不是，就这么简单！
...
不过代码中还存在问题，就是把字典的key值暴露在了外面，这能体现除面向对象的“封装性”吗？！肯定不能啊
