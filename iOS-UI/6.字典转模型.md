...
##### 3.自定义视图
- 回想一下刚刚的创建视图，就是先添加一个UIView，接着在UIView中分别添加UIImageView和UILabel，之后的都是一样的，可不可以抽取出来呢？
- view的封装
...
{
    // 此处的self.frame.size里面的数据是0，所以不能在这里设置
    // 原因是，shopView的frame，在初始化之后才被设置
    if (self = [super init]) {
...
// 调用这个方法，就可以设置self.frame.size的值
- (void)layoutSubviews
{
    [super layoutSubviews];
    
    self.icon.frame = CGRectMake(0, 0, self.frame.size.width, self.frame.size.width);
    self.name.frame = CGRectMake(0, self.frame.size.width, self.frame.size.width, self.frame.size.height-self.frame.size.width);
